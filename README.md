...
它适合实体商家、手工艺品牌和电商运营人员使用。默认少提问、自动执行；原图无法识别、不同图片疑似属于多个 SKU、关键信息冲突，或规格图缺少尺寸时才暂停确认。
...
```text
使用 $generate-ecommerce-product-images，根据我上传的产品图，
自动调研当前热门视觉风格并生成完整电商图片套系。
```
...
规格图至少需要一项用户确认的尺寸字段。Skill 不会根据照片估算尺寸、容量、重量、材质纯度或工艺。用户未提供参数时会主动询问一次；仍未补充则第 07 张标记为等待输入，其他 6 张照常交付。
...
此时第 07 张使用 `availability: "awaiting-user-input"` 的严格等待状态；校验器不会放宽其他 6 张的完整性要求。
